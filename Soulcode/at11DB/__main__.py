...
# Função que vai fazer as colunas com as tabelas:
def contas(bloco):
# Lendo todo o bloco e criando uma lista que contenha apenas a coluna valores para poder fazer os cálculos necessários: 
    valores = []
    cont = 0
    for linha in bloco:
        valores.append(linha[2])
        cont =+ 1 #Variável que serve para identificar a quantidade de registros no bloco, para especifivcar posteriormente a última data
        media = np.mean(valores)
        mediana = np.median(valores)
        desvio_std = np.std(valores)
        moda = st.mode(valores)
        maxi = np.max(valores)
        mini = np.min(valores)
        data_inicio = bloco[0][1]
        data_fim = bloco[cont-1][1]
        row = [media , mediana, moda, desvio_std, maxi, mini, data_inicio, data_fim]
    return row
...
if __name__ == '__main__':
    ...
    # Importando os arquivos do CSV:
        ...
        log("Lendo dados1")
        with open('C:\\at11DB\\DADOS1.csv', newline='') as csvfile:
        ...
        log("Lendo dados2")
        with open('C:\\at11DB\\DADOS2.csv', newline='') as csvfile2:
